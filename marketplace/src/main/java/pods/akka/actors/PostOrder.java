    ...
    private Behavior<Command> onWalletDebitResult(WalletDebitResult msg) {
        if (!msg.success) {
            ...
            if (discountAvailable) {
                ...
                getContext().getSelf().tell(new DiscountUpdated(true, "Discount already availed"));
                /*HttpRequest discountRequest = HttpRequest.newBuilder()
                        ...
                        .build();
                getContext().getLog().info("Sending discount update request for userId {}: {}", userId, discountJson);
                httpClient.sendAsync(discountRequest, BodyHandlers.ofString())
                        .whenComplete((resp, ex) -> {
                            getContext().getLog().info("Discount update response: {}", resp.body());
                            if (ex != null) {
                                getContext().getLog().error("Discount update exception: {}", ex.getMessage());
                                getContext().getSelf().tell(new DiscountUpdated(false, "Discount update failed"));
                            } else if (resp.statusCode() != 200) {
                                getContext().getLog().error("Discount update error: Status {}", resp.statusCode());
                                getContext().getSelf().tell(new DiscountUpdated(false, "Discount update failed"));
                            } else {
                                getContext().getLog().info("Discount updated successfully for userId {}: {}", userId, resp.body());
                                getContext().getSelf().tell(new DiscountUpdated(true, ""));
                            }
                        });
                        getContext().getLog().info("Discount update request sent for userId {}: {}", userId, discountJson);
                        */
            } else {
                getContext().getSelf().tell(new DiscountUpdated(true, "Discount already availed"));
            }
